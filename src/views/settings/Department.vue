                                ...
                                <div class="card-body">
									<DataTable :value="departments" 
                                        ...
                                        <Column field="is_active" header="Status" :filterMenuStyle="{'width' : '14rem'}" style="min-width:12rem">
                                            <template #body="{data}">
                                                <Badge :severity="data.is_active === 0 ? 'danger': 'success' ">{{getStatus(data.is_active)}}</Badge>
                                            </template>
                                             <template #filter="{filterModel}">
                                                <Dropdown v-model="filterModel.value" :options="statuses" placeholder="Any" class="p-column-filter" :showClear="true">
                                                    <template #value="slotProps">
                                                        <span :class="'customer-badge status-' + slotProps.value" v-if="slotProps.value">{{getStatus(slotProps.value)}}</span>
                                                        <span v-else>{{slotProps.placeholder}}</span>
                                                    </template>
                                                    <template #option="slotProps">
                                                        <span :class="'customer-badge status-' + slotProps.option">{{getStatus(slotProps.option)}}</span>
                                                    </template>
                                                </Dropdown>
                                            </template>
                                        </Column>
                                    ...
                                    <Dialog v-model:visible="newDepartmentDialog" :style="{width: '450px'}" header="New Department" :modal="true" class="p-fluid">
                                            <div class="p-field">
                                            ...
                                            <div class="p-field">
                                                ...
                                                <Dropdown id="inventoryStatus" v-model="department.inventoryStatus" :options="statuses" optionLabel="label" placeholder="Select a Status">
                                                    <template #value="slotProps">
...
export default {
    data() {
        return {
           ...
           loading : true,
           breadcrumb : {
                home : { icon: 'pi pi-home',to : '/',label : 'Departments' },
                items : [
                    {label : 'Departments'},
    ...
    mounted() {
        this.ApiService.getDepartments().then(res => {
            ...
            res.data.forEach(element => {
                this.departments.push(element.data)
            });
